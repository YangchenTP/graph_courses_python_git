...
```{python}
pa["status"].replace({"Adulte": "Adult", "Enfant": "Infant"}, inplace = True)
pa["status"].value_counts()
```
...
```{python}
px.box(pa, color = "sex", y = "bmi")
```

Write a short explanation of what you see in the plot.
The plot distributes the BMI of Male vs Female. The male population has a greater range of BMI as compared to the female population.
...
```{python}
def assess_risk(bmi=0,tpa=0):
   if bmi>=30 and tpa<500:
      return "High Risk"
   else:
...
```{python}
px.histogram(pa, x ="risk_category", color = "sex", barnorm = "percent")
```

Comment on the findings. Which gender has a higher proportion of high risk individuals?
The gender FEMALE has a higher proportion of high risk individuals.