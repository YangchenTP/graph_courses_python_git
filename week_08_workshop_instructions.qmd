...
```{python}
px.bar(edu_cost_table, x = "education", y = "mean_total_cost")
```

Write a short interpretation of the plot.
The education level "Graduate" has the highest mean total cost and "No Education" has the least. There is no linear relationship.
...
age_cost_table = tb.groupby("age_group").agg(pat_num = ("age_group", "count"), mean_tc = ("total_cost", "mean"))
...
Write a short interpretation of the plot.
The age group 18-36 has the highest mean total cost, while the age group 48-57 has the least.
...
```{python}
treatment_initiation_delay_in_days = tb.groupby("first_visit_type").agg(total_delays_by_day = ("health_sys_delay_(in_days)", "sum"))

treatment_initiation_delay_in_days = treatment_initiation_delay_in_days.reset_index()
```

```{python}
px.bar(treatment_initiation_delay_in_days, x ="first_visit_type", y = "total_delays_by_day")
```

Findings:
The delay in days is much higher in Private Hospitals as compared to Public Hospitals.

Additional Computation to play around with the data set during the last few minutes of class

```{python}
treatment_initiation_delay_in_days2 = tb.groupby("first_visit_type").agg(
    count_delays=("health_sys_delay_(in_days)", "count"),
    mean_delays=("health_sys_delay_(in_days)", "mean"),
    median_delays=("health_sys_delay_(in_days)", "median"),
    max_delay=("health_sys_delay_(in_days)", "max"),
    min_delays=("health_sys_delay_(in_days)", "min"), range_delays=("health_sys_delay_(in_days)", lambda x: x.max() - x.min())
)
treatment_initiation_delay_in_days2 = treatment_initiation_delay_in_days2.reset_index()
treatment_initiation_delay_in_days2
```

```{python}
px.box(treatment_initiation_delay_in_days2, x="first_visit_type")
```