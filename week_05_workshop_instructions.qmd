...
```{python}
loans_data1 = loans_data.drop(columns=["rownames"])
loans_data1
```
...
```{python}
demog_info = loans_data1[["emp_title", "emp_length", "state", "homeownership"]]
demog_info_renamed = demog_info.rename(columns={"emp_title": "employment_title", "emp_length": "employment_length"})
demog_info_renamed
```
...
```{python}
demog_info_renamed.to_csv('Demographic_Information.csv', index=False)
```
...
```{python}
credit_vars = loans_data1.filter(regex="credit")
...
```{python}
high_income = loans_data1.query("annual_income > 100000")
...
```{python}
high_risk = loans_data1.query("interest_rate > 15 | debt_to_income > 30 |delinq_2y > 0")
...
```{python}
prime = loans_data1.query("(grade == 'A' | grade == 'B') & debt_to_income < 20 & delinq_2y == 0")
prime.to_csv('Prime.csv', index = False)

# we can also use: prime = loans_data1.query(grade.isn(['A', 'B']) & debt_to_income < 20 & delinq_2y == 0)
```
...
```{python}
with pd.ExcelWriter('outputs/loans_data1_subsets.xlsx') as writer:
    demog_info_renamed.to_excel(writer, sheet_name='Demographic_Information_Renamed', index=False)
